...
def get_AAfea_phy():
    AAfea_phy_dict = {}
    in_f = open(os.path.dirname(__file__) + '/assistant_codes/phy/AAfea_phy.txt', 'r')
    for line in in_f:
        cols = re.split('[\t\n]', line)
        if len(cols) < 5:
            continue
...
def blosum50():
    blosum50_matrix = []
    blosum50_file = os.path.dirname(__file__) + '/blosum50.txt'
...
def blosum62():
    blosum62_matrix = []
    blosum62_file = os.path.dirname(__file__) + '/blosum62.txt'
...
def get_embedding():
    ...
    in_f = open(os.path.dirname(__file__) + '/embedding_protein.txt', 'r')
    matrix = []
    for line in in_f:
        line = re.split('[\t\n\[\] ]', line)
        while '' in line:
